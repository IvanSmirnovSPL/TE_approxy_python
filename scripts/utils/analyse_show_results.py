...
class Showing_results:
    ...
    def add_grid(self, tmp):
        ...
        self.r.e1.append(np.log(tmp[0]))
        self.r.e2.append(np.log(tmp[1]))
        self.r.e3.append(np.log(tmp[2]))
    ...
    def show_main_graphics(self):
        for i in range(3):
            residual, name = self.switch_residual_norm(i, self.r)
            plt.clf()
            func_1, func_2 = self.get_func(residual, i)
